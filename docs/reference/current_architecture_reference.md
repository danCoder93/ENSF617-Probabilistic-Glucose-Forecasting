...
Role: Preserved full-length current-state technical reference for this
repository.
Audience: Engineers, contributors, and researchers who need the authoritative
description of the present codebase.
Owns: Preserved long-form reference treatment of package boundaries, runtime and
artifact flow, extension rules, and active constraints.
...
This document preserves the older full-length reference treatment of the
repository as it exists today: the major packages, their ownership boundaries,
the runtime flow, the artifact outputs, and the constraints that are still
intentional.

For most readers, start with [`../current_architecture.md`](../current_architecture.md)
instead. Use this file when you genuinely want the exhaustive, continuous
reference treatment.
...
1. [`../README.md`](../../README.md) for the fast interface
2. [`system_walkthrough.md`](../system_walkthrough.md) for the guided second-pass
   explanation
3. this file for the exact current system shape
4. [`codebase_evolution.md`](../codebase_evolution.md) for the historical why
5. the relevant milestone note in [`history/`](../history/) for subsystem-specific
   depth
6. the generated graphs under [`dependency_graphs/`](../dependency_graphs/) for a
